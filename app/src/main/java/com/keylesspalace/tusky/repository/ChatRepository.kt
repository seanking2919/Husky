...
class ChatRepositoryImpl(
    ...
    private fun addFromDbIfNeeded(accountId: Long, chats: List<Either<Placeholder, Chat>>,
...
fun ChatEntityWithAccount.toChat(gson: Gson) : Chat {
    return Chat(
            account = this.account.toAccount(),
            id = this.chat.chatId,
            unread = this.chat.unread,
            updatedAt = Date(this.chat.updatedAt),
            lastMessage = this.lastMessage?.toChatMessage()
    )
}
